...
### Phase 5: 7게임 데이터 분석 — 완료

**목표**: 7개 MUD 게임의 데이터 구조 + 런타임 메커닉 전수조사

```markdown
✅ tbaMUD 데이터 전수조사 + 런타임 메커닉 분석
✅ Simoon 데이터 전수조사 + tbaMUD 대비 차이점 분석
✅ 3eyes 데이터 전수조사 + 런타임 메커닉 분석
✅ 10woongi 데이터 전수조사 + 런타임 메커닉 분석
✅ muhan13 데이터 구조 분석 + 3eyes 대비 차이점 분석
✅ murim 데이터 구조 분석 + 3eyes 대비 차이점 분석
✅ 99hunter 데이터 구조 분석 + 런타임 메커닉 분석
✅ 통합 갭 분석 (gap-analysis.md) + 범용 데이터 모델 비교 (universal-data-model.md)

결과: 7게임 런타임 포함 전수조사 완료, 통합 설계 기초 자료 확보
```

**분석 대상 7개 게임**:

| 게임 | 엔진 | 파서 구현 | 분석 커버리지 | 런타임 분석 |
|------|------|----------|-------------|-----------|
| tbaMUD | CircleMUD 3.1 | CircleMudAdapter (12파서) | ~92% | 완료 |
| Simoon | CircleMUD 3.0 | SimoonAdapter (11파서) | ~90% | 완료 (차이점) |
| 3eyes | Mordor 2.0 | ThreeEyesAdapter (8파서) | ~85% | 완료 |
| 10woongi | FluffOS LP-MUD | LPMudAdapter (9파서) | ~55% | 완료 |
| muhan13 | Mordor 2.0 | 미구현 | ~95% (struct 해석) | 완료 (차이점) |
| murim | Mordor 2.0 확장 | 미구현 | ~90% (rooms 누락) | 완료 (차이점) |
| 99hunter | SMAUG 1.4 | 미구현 | ~95% (에리어 해석) | 완료 |
...
**어댑터 지원**: C = CircleMudAdapter, S = SimoonAdapter, 3 = ThreeEyesAdapter, L = LPMudAdapter
...
| 테스트 | 전체 통과 | 260 | 260 | 260 | 362 | 달성 |

### Phase 5 (7게임 분석) - 완료

| 기준 | 목표 | 결과 | 상태 |
|------|------|------|------|
| 게임 분석 | 7개 게임 | 7개 완료 | 달성 |
| 런타임 분석 | 대표 4게임 | tbaMUD/3eyes/10woongi/99hunter | 달성 |
| 변종 차이점 | 3게임 | Simoon/muhan13/murim | 달성 |
| 통합 문서 | 2개 | gap-analysis + universal-data-model | 달성 |
| 테스트 | 전체 통과 | 362 | 달성 |
...
### Mordor 2.0 — muhan13 (분석 완료, 파서 미구현)
- 특징: 바이너리 C struct, 3eyes와 동일 크기(1184/352/480)이나 필드 순서 다름
- 난이도: ⭐⭐⭐ (중간) — 3eyes 참고 가능
- 고유: creature2(4096B), 은행/결혼/제련/전쟁/달돌 시스템, board 18개
- 데이터: 3,218 rooms, 907 items, 1,049 mobs, 10 zones, ~228 help

### Mordor 2.0 확장 — murim (분석 완료, 파서 미구현)
- 특징: 바이너리 C struct, creature 3,380B(+185%), 무협 MUD
- 난이도: ⭐⭐⭐⭐⭐ (매우 어려움) — 완전 새 struct
- 고유: 18종 스탯, art[256] 무공, 8계파, 21 직업, 분신법술, 내공심법
- 데이터: rooms 미포함, ~1,077 items, ~683 mobs, 126+ spells
- 주의: rooms/ 디렉토리 누락 (스냅샷 미포함)

### SMAUG 1.4 — 99hunter (분석 완료, 파서 미구현)
- 특징: 단일 에리어 파일, 외부 데이터 파일 (skills.dat/명령어.dat/.class)
- 난이도: ⭐⭐⭐⭐ (어려움) — 새 엔진 계열
- 고유: 30 클래스(6기본+24고급), RIS 내성 시스템, 신/의회, 8-layer 의복
- 데이터: 53 areas, ~2,750 rooms, ~1,750 items, ~1,750 mobs, 488 cmds
...
> 6단계 로드맵 (2026-02-12 기준)

| 단계 | 내용 | 상태 |
|------|------|------|
| **Step 1** | 7개 게임 런타임 분석 (데이터+런타임 전수조사) | **완료** |
| Step 2 | 통합 설계 (DB 스키마 + 프레임워크 데이터 모델) | 다음 |
| Step 3 | 프로토타입 검증 (tbaMUD 1개 게임) | 대기 |
| Step 4 | 7개 게임 전체 마이그레이션 | 대기 |
| Step 5 | 엔진 + 게임 구현 | 대기 |
| Step 6 | 웹 앱 디자이너 도구 | 대기 |

---

**문서 버전**: 2.0
**최종 업데이트**: 2026-02-12 — 7게임 분석 Phase 5 완료, 로드맵 추가, 신규 3게임(muhan13/murim/99hunter) 부록 추가, 362 tests
